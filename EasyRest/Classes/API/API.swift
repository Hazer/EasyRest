...
open class API <T> where T: Decodable {
    ...
    public init(path: URL, method: HTTPMethod, queryParams: [String: String]?, bodyParams: Body?, headers: [String: String]?, interceptors: [Interceptor]?) {
    ...
    open func upload(_ onProgress: @escaping (_ progress: Float) -> Void, onSuccess: @escaping (_ result: Response<T>?) -> Void,
        ...
        if case Body.params(let bodyParams) = self.body! {
            assert((bodyParams?.count ?? 0) == 1)
            
            self.beforeRequest()
            
            Alamofire.upload(multipartFormData: {form in
                for (key,item) in bodyParams! {
    ...
    open func execute( _ onSuccess: @escaping (Response<T>?) -> Void, onError: @escaping (RestError?) -> Void, always: @escaping () -> Void) {
        ...
        switch body! {
        case let .params(bodyParams):
            request = manager.request(path.url!, method: self.method, parameters: bodyParams, encoding: JSONEncoding.default, headers: headers)
        case .codable(let codablea):
            request = manager.request(path.url!, method: self.method, parameters: codablea, encoding: JsonEncodableParameters.default, headers: headers)
        default:
            fatalError("Request failed to create.")
        }
